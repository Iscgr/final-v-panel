-- Migration: threshold_config (Phase C – E-C4 Dynamic Thresholds)
-- Status: ACTIVE 
...
CREATE TABLE threshold_config (
  ...
  critical_threshold NUMERIC(18,6) NOT NULL,      -- must be >= warn_threshold 
...
-- Seed data with initial thresholds
INSERT INTO threshold_config(metric_code, warn_threshold, critical_threshold, window_minutes, meta) VALUES
('outbox_failure_rate', 1.0, 2.0, 60, '{"unit": "percent", "description": "Telegram message failure rate"}'),
('outbox_avg_retry', 1.5, 2.5, 60, '{"unit": "count", "description": "Average retry attempts per message"}'),
('outbox_latency_p95', 5000, 8000, 60, '{"unit": "milliseconds", "description": "95th percentile latency"}');
...
CREATE INDEX idx_threshold_config_metric ON threshold_config(metric_code);