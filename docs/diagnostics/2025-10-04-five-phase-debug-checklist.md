...
**Frontend**
- [x] ردیابی مسیر کامپوننت‌های تب (به احتمال `client/src/pages/admin/SalesPartners.tsx`).
- [x] تکمیل دکمه «همکار جدید» با فرم، اعتبارسنجی، و ارسال واقعی.
- [x] حذف پیام placeholder «این قابلیت در دست توسعه است» و جایگزینی با فرم ویرایش فعال.
- [x] رفع خطای فعلی بارگذاری تب همکاران فروش و تضمین مصرف اندپوینت‌های صحیح. *(1404/07/12 – Query `useQuery`، مدیریت خطا و تضمین مسیر `/api/representatives`)*
- [x] افزودن جدول رویدادهای پورسانت (پرداخت/تسویه) با فیلتر زمانی. *(1404/07/12 – تب `commission` با فیلتر نماینده/تاریخ و اتصال به `/api/sales-partners/payments`)*
